...
public class SimRank {
	...
	private float simRank(Integer id1, Integer id2) {
		...
		Set<Integer> inlinks1 = graph.inLinks(id1);
		Set<Integer> inlinks2 = graph.inLinks(id2);
		...
		for(int i:inlinks1){												
			for(int j:inlinks2){
				score += getSimRank(i,j);
			}	
		}
		// score = Float.valueOf(( damping / ( numInLinks1 * numInLinks2 ) ) * score );
		score = (float) (( damping / ( numInLinks1 * numInLinks2 ) ) * score);
