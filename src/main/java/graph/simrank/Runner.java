	...
	public void simRankSimilarity(){
		...
		for(Integer k1:keySet){					
			...
			try {					
				...
				//String file = this.getClass().getClassLoader().getResource(this.resultDir + str1 + ".txt").getFile();

				File file = new File(this.resultDir + str1 + ".txt");
				if (!file.exists()) {
					file.createNewFile();
				}
