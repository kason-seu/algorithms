...
public class Manachor2 {
    ...
    public static void main(String[] args) {
        ...
        Manachor2 manachor = new Manachor2();
    ...
    private Pair broad(int[] arr, int i, String str) {

        int remove = 1;

        boolean flag = false;

        while (i - remove >= 0 && i  + remove < str.length()) {
            if (str.charAt(i-remove) == str.charAt(i  + remove)) {

                flag = true;
                remove = remove + 1;
            } else {
                arr[i] = remove - 1;
                return new Pair(i,  i + remove - 1);
            }
        }
        if (flag) {
            arr[i] = remove - 1;
